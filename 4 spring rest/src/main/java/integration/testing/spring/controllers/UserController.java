package integration.testing.spring.controllers;

import integration.testing.spring.exceptions.*;
    ...
    User patchUser(@RequestBody Map<String, String> updates, @PathVariable Long id) {
        ...
        return repository.findById(id)
                .map(user -> {
                    ...
                    if (null != registered) {
                        user.setRegistered(registered.equalsIgnoreCase("true") ? true : false);
                    }
                    return repository.save(user);
                })
                .orElseGet(() -> {
